  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 text-slate-800 space-y-3 shadow-xs">
      ...
      <div className="pt-2.5 border-t border-amber-200/60 flex items-center justify-between text-xs text-amber-700 flex-wrap gap-2">
        <span>Enterprise Grounding Policy: Refusing ungrounded generation.</span>
        <button
          type="button"
          onClick={onReset}
...
export function ErrorNotice({ errorMessage, onRetry }: ErrorNoticeProps) {
  return (
    <div className="bg-red-50 border border-red-200 rounded-lg p-5 text-slate-800 space-y-3 shadow-xs">
      <div className="flex items-start space-x-3">
      ...
      <div className="pt-2.5 border-t border-red-200/60 flex items-center justify-between flex-wrap gap-2">
        <span className="text-xs text-red-700">Check API client configuration or server connectivity.</span>
        <button
          type="button"
          onClick={onRetry}
          className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded bg-red-600 text-white text-xs font-medium hover:bg-red-700 transition-colors shadow-xs"
        >
  ...
  return (
    <div className="bg-slate-100 border border-slate-200 rounded-md p-3 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs">
      <div className="flex items-center space-x-2 text-slate-600">
        ...
        <span className="font-semibold text-slate-900">API Contract Test Switcher:</span>
        <span className="hidden md:inline text-slate-500">
          Simulate backend contract scenarios
        </span>
      </div>
